...
func TestFindLatestCell(t *testing.T) {
	prepareTestDatabase()
...
func TestFindAllLatest(t *testing.T) {
	prepareTestDatabase()

	expected := "185133a4-71e4-4595-8f09-3ffe416064fc"

	cells, err := ds.FindAllLatest("BASIC_INFO")

	if err != nil {
		t.Error(err)
	}

	if (*cells)[0].RowID != expected {
		t.Error(fmt.Sprintf("Expected first address rowid to be %s, got %s", expected, (*cells)[0].RowID))
	}
}
